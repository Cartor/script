...
# iOS App Store Icons
if [[ $FILE != "./iTunesArtwork@2x" || $iOSSubdir != "" ]]; then sips --resampleWidth 1024 "${DIR}/${FILE}" --out "${DIR}${iOSSubdir}/iTunesArtwork@2x" > /dev/null 2>&1; fi
sips --resampleWidth 512 "${DIR}/${FILE}" --out "${DIR}${iOSSubdir}/iTunesArtwork" > /dev/null 2>&1
...
sips --resampleWidth 228 "${DIR}/${FILE}" --out "${DIR}${iOSSubdir}/Icon-76@3x.png" > /dev/null 2>&1
...
# Android icons
sips --resampleWidth 512 "${DIR}/${FILE}" --out "${DIR}${androidSubdir}/Icon-web.png" > /dev/null 2>&1
sips --resampleWidth 144 "${DIR}/${FILE}" --out "${DIR}${androidSubdir}/Icon-xxhdpi.png" > /dev/null 2>&1
sips --resampleWidth 96 "${DIR}/${FILE}" --out "${DIR}${androidSubdir}/Icon-xhdpi.png" > /dev/null 2>&1
sips --resampleWidth 72 "${DIR}/${FILE}" --out "${DIR}${androidSubdir}/Icon-hdpi.png" > /dev/null 2>&1
sips --resampleWidth 48 "${DIR}/${FILE}" --out "${DIR}${androidSubdir}/Icon-mdpi.png" > /dev/null 2>&1
sips --resampleWidth 36 "${DIR}/${FILE}" --out "${DIR}${androidSubdir}/Icon-ldpi.png" > /dev/null 2>&1
